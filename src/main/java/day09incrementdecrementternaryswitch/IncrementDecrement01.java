package day09incrementdecrementternaryswitch;

public class IncrementDecrement01 {
    public static void main(String[] args) {

        //Increment
        int a = 5;
        System.out.println(a);
        a = a + 2; // veya a+=2
        System.out.println(a);

        //EX: bir tane integer variable olusturun ve iki yolla da 5 artirin.

        int b = 19;
        b=b+5;
        b+=5;
        System.out.println(b);

        //Decrement
        int c=8;
        System.out.println(c);//8
        c=c-3;
        System.out.println(c);//5
        c-=3;
        System.out.println(c);//2

        //Increment 2:
        int d=6;

        d=d+2;
        System.out.println(d); //12

        d*=2;
        System.out.println(d);//24

        //Decrement 2:
        int e=24;

        e=e/2;
        System.out.println(e);//12

        //1 ile increment ve decrement

        int f =12;

        //f=f+1;
        //f+=1;
        f++;

        //1 ile decrement

        int h=10;

        //h=h-1;
        //h-=1;
        h--;

        //Post increment ve Pre increment

        int i =10;
        int k=i++; //post-increment cünkü increment, variable'in isminden sonra yazildi.

        System.out.println(k); //10 ==>"i" artirilmadan "k"ya konuldugu icin "k"nin degeri 10 olur.
        System.out.println(i); //11 ==>"i" satir sonunda artirildigi icin "i" nin degeri 11 olur.



        int m=15;
        int n=++m; //post-increment cünkü increment, variable'in isminden önce yazildi.

        System.out.println(m);//16 ==>"m" satir sonunda artirildigi icin "m" nin degeri 16 olur.
        System.out.println(n);//16 ==>"m"artirildiktan sonra  "n" ye konuldugu icin "n" nin degeri 16 olur.












    }//main Body
}//Class body

